    ...
    if R21 is not None and len(R21):
        ...
        (TABLES / "regions.tex").write_text(
            tex_table(d.sort_values(["log", "target"]),
                      ...
                      "draws. THESE ARE THE LABELS THE BAND YIELDS BEFORE "
                      "THE MINIMUM RESOLVED SHARE of Definition~"
                      "\\ref{def:regions}, which changes the label on "
                      "\\nMarkedBelowMinShare\\ of these rows; "
                      "Table~\\ref{tab:master} and Table~\\ref{tab:triple} "
                      "print the labels the article quotes. No "
