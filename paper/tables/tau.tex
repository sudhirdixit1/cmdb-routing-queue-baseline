...
\caption{The decision time implemented as an axis. Each moment carries its own population, its own register and its own admissible set; the PREDICTED TARGET is the same at all of them. $\tau_0$ and $\tau_1$ differ only in which cases are in scope. $\tau_1$ and $\tau_2$ differ in what is known AND in which cases are in scope, because 942 incidents have no interaction record; the fourth row is $\tau_2$ restricted to $\tau_1$'s own population, and it is that row, not the third, that makes the $\tau_1$-to-$\tau_2$ step an information step alone. Every row is on the estate's own cohort and the reassignment target, under a declared tie-break, and every interval is the pointwise basic construction at \nTauDraws\ draws.}
\label{tab:tau}
\end{table}
