...
\caption{Calibration and unseen-category rates at the reference cell, one row per log--target pair in the same order as every other per-pair table. Every row is on the cohort and target the registered rules of Section~\ref{sec:design} define, BPIC14's included; the case study's own cohort is smaller and its counts are in Section~\ref{sec:cohort}.}
