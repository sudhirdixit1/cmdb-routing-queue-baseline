...
| **D4** | "Everything is on BPI Challenge 2014. 'Each choice moves the answer by more than the effect' is an n = 1 claim." | §10; §11 | The full four-axis surface on **all three logs** whose reduction is resolvable — three organisations, three information systems, two domains. On every one, the baseline choice alone moves *R* by more than *R* itself. Plus a simulation whose true answer is known by exhaustive enumeration. |
| **D5** | "Twelve self-reported errors. Why should I trust the thirteenth number?" | §16; Appendix B | The main text reports the **defect classes**, not the incidents; the incidents are in an appendix. 1,358 checks against regenerated result files, 0 failed; a corruption suite of 243 mutations that has found every hole this apparatus has ever had, including three this round. |
| **D6** | "Forty pages, single unaffiliated author, no deployment. Is this the right journal?" | `DECISIONS.md` §15; §17 Limitations | The venue decision was **re-taken** against six criteria fixed in advance, and criterion 6 was counted rather than assumed: of 3,360 recent articles across eight candidate venues, **five are single-author and unaffiliated**. That is in the Limitations and in the cover letter, not in a hope. |
...
| "Would you accept this audit if your own paper were in the sample?" | `results/r40_coding.csv` | Every row carries the DOI, the quote and the PDF page, so any row can be checked in a minute. The adjudication extracts are in `data/audit/adjudication/`. |
