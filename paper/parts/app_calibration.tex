...
Under the calibrated band the region counts --- the labels
Definition~\ref{def:regions} yields, minimum resolved share included, which
are the ones Table~\ref{tab:master} prints --- are: conditionally beneficial,
\nCondBeneficialMinShare; sign-changing, \nSignChangingMinShare;
conditionally harmful, \nCondHarmfulMinShare; unresolved,
\nUnresolvedMinShare; and the median robustness index is
\rhoMedianCalibrated. Before the minimum share is applied the same band gives
\nCondBeneficialCal, \nSignChangingCal, \nCondHarmfulCal\ and
\nUnresolvedCalibrated\ in that same order, which is the region column of
Table~\ref{tab:calbands}; the difference is the \nLabelsLostToMinShare\
