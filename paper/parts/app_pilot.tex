...
a public index across \nAuditVenues\ venue strata, two citation strata and a
topic stratum; the enumeration is cached per stratum in the archive so it can
be re-run without re-querying. A stratified probability sample was drawn with
an allocation declared before the enumeration finished --- proportional to
stratum size with a floor of \minPerStratum\ per stratum --- and every
estimate below carries the design weight $N_h / n_h$.
...
to about $\pm\auditHalfWidthPct$. A design that resolved it to $\pm 7$
percentage points would need \nAuditRequired\ adjudicated eligible papers.
