...
# 22. Round nineteen — the *Information Systems* reject, answered

## 22.1 What arrived

An external referee report on the round-eighteen manuscript, recommending
**reject as submitted** for *Information Systems*. Ten major comments. Its own
summary: several weaknesses affect the central evidence rather than the
presentation, and a strong version would require *a redesigned study and a
substantially new manuscript*.

That was taken literally. This round is a new manuscript
(`paper/specification_surfaces.tex`, assembled from `paper/parts/`) under a
new title, with a new estimand, four new methodological objects, a rebuilt
inference layer, a rebuilt empirical design, and the literature component
demoted to a measured pilot. The round-eighteen manuscript is retained.

## 22.2 The architectural change: one source of truth for every number

The single most useful thing this round did is not statistical.

**The manuscript contains no numeric literals.** Every number is a macro in
`paper/numbers.tex`, generated by `scripts/make_numbers.py` from
`results/*.csv`; every data table is generated too. `scripts/texlint.py` fails
the build if a bare number appears in the prose.

This removes, structurally, the defect class that produced the referee's first
major comment: an abstract that disagreed with section 10 of the same
manuscript. It cannot recur, because there is one of each number.

It introduces a new failure mode — a wrong generator makes the whole
manuscript wrong *consistently* — and `scripts/verify_numbers.py` is the guard
against that: it re-derives each macro from its source with code written
against the definition rather than against the generator.
`scripts/s13_attack_numbers.py` is that guard's own regression suite.

## 22.3 What the evidence now says, and where it contradicts the previous round

| claim | round eighteen | round nineteen |
|---|---|---|
| the headline spread | "the baseline choice moves R by more than R itself", on a comparison inflated by an intercept-only rung | the intercept-only rung is excluded by definition; the baseline axis moves the increment by 0.091 AUC at the median pair, 0.298 at the widest |
| which axis dominates | the baseline | **none does.** The largest median first-order index is 10.1%; the interactions carry a median 30.0%, and which axis leads varies by pair |
| what one number costs | not measured | a conventional report misstates the **sign** for a median 35.3% of admissible specifications; above 10% on 18 of 19 pairs |
| the surface report's advantage | asserted | measured under one loss: the *conservative* rule is **worse** than one number; the *majority* rule beats it by about a fifth |
| register population | asymmetric between tail and core | **symmetric** on this cohort, to three decimals |
| staleness | not measured | **exactly zero**, and the reason is the encoder, not the register |
| "not one of twenty papers…" | asserted | **withdrawn.** The screen's measured sensitivity is under one half |

Every one of those is a correction and is in Appendix C of the new
manuscript, grouped by class.

## 22.4 Three things that cost hours and are worth knowing

**OpenMP oversubscription.** Twelve worker processes each spawning fourteen
OMP threads on a fourteen-core machine turned a 3-second gradient-boosting fit
into an 8-second one, and a 35-minute run into a four-hour one. Every script
with a process pool now sets `OMP_NUM_THREADS=1` and its four siblings
*before* importing numpy. The lesson generalises: parallelism at two levels
multiplies, it does not add.

**`data/` versus `data/*` in .gitignore.** Git does not descend into an
ignored *directory*, so a blanket `data/` makes every later negation dead.
Round eighteen ignored `data/` and the manuscript claimed the adjudication
extracts were committed; `git ls-files data/` returned nothing. Fixed, and the
claim is now true for the adjudications and correctly *not* made for the
dossiers, which are extracts of other people's copyrighted text and are
regenerated rather than redistributed.

**Python's `hash()` is salted per process.** Seeding a simulation world with
`hash(world_name)` gives every worker a *different* world with the same name,
so the truth computed in the parent is not the truth simulated in the child.
The world seeds are literals now.

## 22.5 What is still open

- **A Zenodo DOI for the frozen release.** The archive metadata is written
  (`.zenodo.json`); minting the DOI needs the repository owner's Zenodo
