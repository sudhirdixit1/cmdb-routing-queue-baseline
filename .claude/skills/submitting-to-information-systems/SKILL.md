...
Everything the repository can do is done and gated, and as of 2026-09-06
the DOI is minted and inserted. What remains is the postcode, Elsevier's
forms, and the upload itself. The one command that says what
...
| 5 | Built image's digest | **Decided 2026-09-06: not owed.** No built image is published; the code-availability statement and `REPRODUCE.md` say the image is built from the archive's Dockerfile (base pinned by digest) and that none is distributed. `python scripts/finalise.py --image-digest sha256:<64 hex>` still works if one is ever pushed | `finalise.py` does not list it |
| 6 | Reviewer e-mails | **Done 2026-09-06**: six institutional addresses in `submission/suggested_reviewers.md`, each with its source (own institutional page or a 2022+ corresponding-author line); the file was rewritten for the current manuscript. Re-verify affiliations if the submission slips by months | six addresses present |
| 7 | Postal address | **Street, city and state done 2026-09-06** (`AFFILIATION_STREET`, `AFFILIATION_REGION` beside `AFFILIATION_CITY` in `scripts/round26_numbers.py`); the **postcode is still owed**: add it to `AFFILIATION_REGION` (e.g. `NC 275xx`), then `python scripts/make_numbers.py --strict && python scripts/assemble_paper.py && python scripts/build_journal.py` | title page shows the full line |
