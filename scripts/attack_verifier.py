...
CORRUPTIONS = [
    ...
    ("soften the falsification qualifier",
     ...
     r"The claim registered in \texttt{PROTOCOL.md} \S8 is qualified"),
    ("delete the no-power admission",
     "on $10$ of the $19$ pairs the reduction has no denominator",
     "on $2$ of the $19$ pairs the reduction has no denominator"),
    ("make the headroom peek matter",
     r"changes \textbf{no} admission decision on any of the $26$",
     r"changes \textbf{no} admission decision on any of the $16$"),
    ("corrupt the permitting cardinalities",
     "opening resource stamp has cardinality $7$ to $18$",
     "opening resource stamp has cardinality $7$ to $88$"),
    ("make the temporal null reproduce the finding",
     "They reach base AUC at most $0.6483$",
     "They reach base AUC at most $0.8483$"),
    ("shrink the temporal null's gap",
     "the real field is $0.1558$ above that",
     "the real field is $0.0158$ above that"),
    ("turn the contiguous-block null into the one we ran",
     "We did not build cells from contiguous blocks of the time-ordered",
     "We built cells from contiguous blocks of the time-ordered"),
    ("delete the subset admission",
     r"One thing this ladder does \emph{not} do is condition on a subset",
     "One thing this ladder does is condition on a subset"),
    ("move the harmful band away from a real desk",
     "so the harmful band is where a capacity-constrained desk sits",
     "so the harmful band is where no real desk sits"),
    ("corrupt the harmful band's acted share",
     r"group-aware item model acts on $17\%$ to $37\%$ of arrivals",
     r"group-aware item model acts on $77\%$ to $97\%$ of arrivals"),
    ("reverse the subsample concession",
     "it is a sensitivity, not a replication",
     "it is a replication, not a sensitivity"),
    ("overstate what the buyer should conclude",
     "We do not say that about registers in general",
     "We say that about registers in general"),
    ("swap the metric-relativity example's instrument",
     r"``$60.3\%$'' under average precision are not one quantity",
     r"``$60.3\%$'' under Brier skill are not one quantity"),
    ("delete the bootstrap's scope admission",
     "It therefore quantifies test-set sampling error and not model-fitting",
     "It therefore quantifies model-fitting"),
    ("claim four is the number of choices that exist",
     r"four is a claim about how many choices are usually left \emph{implicit}",
     "four is a claim about how many choices exist"),
    ...
    ("make the null reproduce the finding",
     r"reach base AUC at most $0.6479$",
    ...
    ("corrupt the intake-mix sweep",
     r"moves the reduction from $95.3\%$ to $6.3\%$",
    ...
    ("corrupt the u_symptom figures",
     r"is $3.4\%$ unique with a mean length of $10.9$ characters",
