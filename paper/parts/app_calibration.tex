This section derives a critical value calibrated at the pair's own ratio of
register cardinality to training row count. \textbf{The article does not
apply it.} The factor was fitted to restore \emph{pointwise} coverage on a
plane built around a different inference surface, and
Section~\ref{sec:simband} measures the family-wise widening the article's
families need as larger than this factor supplies, so applying it would spend
resolution and still not earn the level. The construction is given in full
because the measurement that retired it is only legible beside it, and because
the sensitivity of every downstream count to it is reported below.
...
Under the band the article reports --- no widening applied --- the region
counts, the labels Definition~\ref{def:regions} yields with the minimum
resolved share included, which are the ones Table~\ref{tab:master} prints,
are: conditionally beneficial, \nCondBeneficialMinShare; sign-changing,
...
direction to withdraw (Table~\ref{tab:triple}). One of those is the only
conditionally harmful label the band reaches, so under
Definition~\ref{def:regions} this corpus carries no conditionally harmful
surface at all.

\noindent Had the widening been applied instead, the same definition would
give \nCondBeneficialCal, \nSignChangingCal, \nCondHarmfulCal,
...
resolution would cost --- which is the reckoning Section~\ref{sec:simband}
says the corpus cannot afford to call a correction, because the widening
these families need is larger still.
...
\nResolvedWholeMult. \textbf{The first of those four is the one the article
reports}: the calibrated and upper-end rungs are a sensitivity ladder showing
what a widening would cost, not settings the corpus runs under, because none
is applied (Section~\ref{sec:simband}). That last comparison is not a
calibration setting either but a different estimator of the same quantity, and
it moves the resolution by \emph{less} than the calibration would ---
\nResolvedEmpBand\ cells against the calibrated rung's
\nResolvedSensCalibrated, from a nominal \nResolvedSensNominal. What moves across all four is how much resolution
the corpus has; what does not move is which pair reaches $\rho = 1$: the
count is \nUnifBenSensNominal\ at $c = 1$, \nUnifBenSensCalibrated\ under
the calibrated factors and \nUnifBenSensUpper\ with every factor at the
upper end of its own Monte Carlo interval, and \nUnifBenEmpBand\ under the
empirical critical value --- the one construction Section~\ref{sec:simband}
measures at its nominal level --- and it is the same pair every time. Across the corpus \nRegionsChangedByCalibration\ region labels change
under the widening, and that one is not among them. The
