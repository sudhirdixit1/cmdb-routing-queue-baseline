    ...
    if G is not None and len(G):
        ...
        (TABLES / "calbands.tex").write_text(
            tex_table(d, "Resolution regions under the nominal critical "
                         ...
                         "operative critical value the article's labels rest "
                         "on (Section~\\ref{sec:simbands} says which "
                         "estimator that is); "
                         "the labels are computed at the operative critical "
                         "value of Section~\\ref{sec:simbands}, which is "
                         "the value $c$ multiplies. The "
    ...
    if C is not None and len(C):
        ...
        (TABLES / "plane.tex").write_text(
            tex_table(d, "The coverage calibration. For each cell of the "
                         ...
                         "computed, not searched for. THIS CALIBRATION IS "
                         "DERIVED AND REPORTED, NOT APPLIED: it was fitted to "
                         "restore POINTWISE coverage on this plane, and "
...
def write_round25(mn):
    ...
    (TABLES / "bandcov.tex").write_text(
        tex_table(d, "Family-wise coverage of the max-$t$ band against a "
                     ...
                     "one it runs. EVERY ROW IS A FAMILY THE CORPUS RUNS: "
                     "\\textsf{whole-surface} at $K = \\nCellsPerPairMedian$ "
                     "and \\nDrawsSurface\\ draws is every pair's surface "
                     "family, \\textsf{decision-curve} at $K = "
                     "\\nDcaFamilyModal$ and \\nDrawsSurface\\ draws the "
                     "modal curve family, and the \\nDcaFamily-cell row at "
                     "\\nDcaDraws\\ draws the case study's own curve family; "
                     "the draw-count ladder the text quotes is a "
                     "sensitivity around the first and is not printed here. "
                     "Nominal is $0.95$; the five "
                     ...
                     "families do not; \\textsf{nonzero} is heavy with a fixed "
                     "heterogeneous non-zero truth, the regime under which "
                     "a cell can be resolved correctly and a region label "
                     "is actually being asked to hold. Every cell is "
